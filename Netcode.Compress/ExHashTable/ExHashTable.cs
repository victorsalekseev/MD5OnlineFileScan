...
using Netcode.Classes.Compress;
    ...
    public class ExHashTable
    {
        ...
        public bool AddNewHtSign(string signature, bool write_ok_evnt)
        {
            ...
            try
            {
                ...
                if (!ht_base.ContainsValue(signature) &&
                    !ht_new.ContainsValue(signature))
                {
                    ht_new.Add(signature, signature);
                    return_val = true;
                }
                else
                {
        ...
        public void AddNewHtSignsToHtBase(int prev_cnt)
        {
            try
            {
                ...
                {
                    if (!ht_base.ContainsValue(ht_new[key]))
                    {
                        ht_base.Add(key, ht_new[key]);
                    }
                }
                if (OK != null)
                {
        ...
        public bool LoadNewHtSighInDB_upd(string db_update, ProgressBar pb)
        {
            try
            {
                ...
                {
                    if (!ht_base.ContainsValue(ht_update[key]))
                    {
                        ht_base.Add(key, ht_update[key]);
                    }
                }
